    ...
    <section className="bg-[#2C4A2E] px-4 py-16 text-[#F5F0E8] sm:px-6 lg:px-8">
      <div
        className={`mx-auto max-w-6xl gap-8 ${
          compact ? "grid lg:grid-cols-[1.1fr_0.9fr] lg:items-center" : "grid md:grid-cols-[1fr_1.1fr] md:items-center"
        }`}
      >
        <div>
        ...
        <form
          ...
          className="grid gap-3 rounded-sm border-2 border-[#F5F0E8]/25 bg-[#1C1C1A]/20 p-4"
          aria-busy={state === "loading"}
        >
          <label className="text-xs font-bold uppercase tracking-[0.14em]" htmlFor={`${formId}-email`}>
            {panelLabel}
          </label>
          <input
            id={`${formId}-email`}
