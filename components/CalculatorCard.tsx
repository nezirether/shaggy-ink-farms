...
export function CalculatorCard({ calculator }: CalculatorCardProps) {
  ...
  return (
    <Link
      ...
      {...(calculator.isExternal ? { target: '_blank', rel: 'noopener noreferrer' } : {})}
    >
      <div className="flex h-full flex-col rounded-sm border-2 border-[#1C1C1A] bg-[#D7D4CC] p-5 shadow-[4px_4px_0_rgba(44,74,46,0.12)] transition hover:shadow-[6px_6px_0_rgba(44,74,46,0.2)] hover:-translate-y-0.5">
        {/* Badge row */}
        <div className="mb-3 flex items-center gap-2">
          ...
          {calculator.badge && (
            <span className="rounded-sm border border-[#C6933F] px-2 py-0.5 text-[10px] font-bold uppercase tracking-[0.1em] text-[#C6933F]">
              {calculator.badge}
            </span>
          )}
        </div>
